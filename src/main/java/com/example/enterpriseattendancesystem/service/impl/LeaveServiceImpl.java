...
public class LeaveServiceImpl extends ServiceImpl<LeaveMapper, Leave> implements ILeaveService {
    ...
    @Override
    public String saveLeave(Long id, String startDate, String endDate, String leaveReason) {
        LocalDateTime startTime = LocalDate.parse(startDate, DateTimeFormatter.ISO_LOCAL_DATE).atStartOfDay();
        LocalDateTime endTime = LocalDate.parse(endDate, DateTimeFormatter.ISO_LOCAL_DATE).atStartOfDay();
        ...
        leave.setEmployeeId(Math.toIntExact(id));
        ...
        if (startTime.isBefore(endTime) && !startTime.toLocalDate().isBefore(currentTime.toLocalDate())) {
            ...
            return "请假申请提交成功，时长：" + duration + "小时";
        } else {
    ...
    @Override
    public IPage<Leave> findAll(String status, int pageNum, int pageSize) {
